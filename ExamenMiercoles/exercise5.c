...
int main(int argc, char** argv)
{
  ...
  char nombre[256];
  char nombre2[256]= "MergeSortProf10.txt";
  char nombre3[256]= "QuickSortProf10.txt";
  ...
  /*compute times*/
  ret =generate_sorting_times(QuickSortProf,nombre3 ,num_min, num_max,incr, n_perms);
  if (ret == ERR) { 
    printf("Error in function generate_sorting_times Quick\n");
    exit(-1);
  }
  ret =generate_sorting_times(QuickSortProfAux,nombre ,num_min, num_max,incr, n_perms);
  if (ret == ERR) { 
    printf("Error in function generate_sorting_times Quick\n");
    exit(-1);
  }
   ret =generate_sorting_times(MergeSortProf,nombre2 ,num_min, num_max,incr, n_perms);
  if (ret == ERR) { 
    printf("Error in function generate_sorting_times Merge\n");
    exit(-1);
  }
 /*
  ret = 
